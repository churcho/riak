...
| MapReduce | `/mapred` | `POST` (plus `GET/HEAD` usage response) | `200` success; `400` request/phase validation; `500` execution/sink errors; `503` timeout/operator-disabled | `Content-Type: application/json`; with `chunked=true` return `multipart/mixed` | `chunked=true`, request JSON with `inputs` and `query` | invalid JSON/request -> `400`; timeout -> `503`; pipeline/runtime errors -> `500` | `riak_kv_wm_mapred` |
