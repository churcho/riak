...
| If-Match / If-Unmodified-Since enforcement | PUT/POST requests with these headers now receive 412 Precondition Failed when conditions aren't met. If-None-Match now only accepts `*`; entity-tag values return 400 `invalid_if_none_match`. | Always active when headers present | Previously these headers were accepted but not enforced. Clients relying on unconditional writes despite sending conditional headers may see new 412 responses. Remove conditional headers to restore unconditional behavior. Clients sending entity-tag values in If-None-Match (e.g., `"etag-1"`) must switch to `*` or remove the header. |
